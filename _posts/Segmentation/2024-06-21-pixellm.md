...
* **Visual Encoder $\mathcal{I}$** (CLIP-ViT-H): $I_ {\text{img}} = \lbrace I_ {\text{img}} ^ \ell \rbrace _ {\ell=1} ^ L = \mathcal{I} (x_ {\text{img}})$ ($L$: visual scales = layers)
* **LLM $\mathcal{F}$**: $y_ {\text{res}} = \mathcal{F} (p_ {V \rightarrow T} (I_ {\text{img}} ^ I), x_ {\text{txt}}, C_ {\text{seg}})$
