...
그리고 **각 $\sigma$에 대한 loss의 비율, weight function** $\lambda(\sigma)$도 결정해야 한다. $\sigma_i$에서 $i$가 증가할수록 true data distribution에 가까워지므로 더 중요하여, $\lambda(\sigma)$도 이에 따라 증가하는 함수로 정한다. 일반적으로 $\lambda(\sigma_i) = \sigma_i^2$로 설정한다. 이때 loss function은 다음과 같이 정리된다.
...
# 3. Controllabe Generation Process
