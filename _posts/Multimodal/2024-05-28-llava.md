...
**LLaVA**는 처음으로 **instruction tuning을 VLM에 적용**한 모델로 의미가 있다. 이 논문이 나온 뒤로 굉장히 많은 후속 논문이 쏟아졌기에 영향력이 지대하다 할 수 있다. 이러한 **vision instruction tuning**은 모델이 **더 정확하고 자세하게 시각적 상황을 이해하고 사람의 instruction에 적합하게 대답하도록** 만들었다. 모든 것이 open-source라는 점도 중요하다. 아쉬운 점은 굉장히 **간단한 MLP-style modality interface**인 projection matrix $\mathbf{W}$를 사용했다는 점이다. 이후 논문에서는 이러한 부분이 개선되었다.
