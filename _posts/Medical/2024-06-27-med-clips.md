...
이 논문은 Microsoft Research에서 2023년에 발표한 논문으로, **BiomedCLIP**이라는 **pre-trained biomedical foundation model**을 제안한다. 지금까지의 데이터셋은 기껏해야 200K 정도였는데, 본 논문에서는 PubMed로부터 Figure-caption 정보를 추출하여 **15M**의 image-text pair를 구축하였다.
